...
import { splashComplete, getSplashIsComplete, getInstanceId, getInstanceTempDir } from './store';
import { myStudyUrl } from './urls';

export const getInstanceInfo = () => {
  return {
    instanceId: getInstanceId(),
    tempDir: getInstanceTempDir(),
  };
};
...
export const relaunch = () => {
  if (config.isolatedStorage) {
    try {
      ...
      console.log(`重启应用，将创建新的实例，当前实例ID: ${instanceId}`);
      
      // 在重启前尝试清理临时目录
      const tmpDir = getInstanceTempDir();
      if (tmpDir && tmpDir.includes(instanceId)) {
        console.log(`重启前清理临时目录: ${tmpDir}`);
        // 临时目录会在应用退出时自动清理，这里不需要手动清理
      }
    } catch (error) {
      console.log('准备重启失败:', error);
    }
  }
